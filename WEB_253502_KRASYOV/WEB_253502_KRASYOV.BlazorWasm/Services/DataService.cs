        ...
        public async Task GetProductListAsync(string? categoryNormalizedName, int pageNo = 1)
        {
            ...
            if (tokenRequest.TryGetToken(out var accessToken))
                _httpClient.DefaultRequestHeaders.Authorization = new 
                    System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", accessToken.Value);
            try
            {
                var route = new StringBuilder($"{_httpClient.BaseAddress.AbsoluteUri}Devices/");

                if (categoryNormalizedName is not null) route.Append($"{categoryNormalizedName}/");

                List<KeyValuePair<string, string>> queryData = new();

                if (pageNo > 1) queryData.Add(KeyValuePair.Create("pageNo", pageNo.ToString()));

                if (!_pageSize.Equals("3")) queryData.Add(KeyValuePair.Create("pageSize", _pageSize));

                if (queryData.Count > 0) route.Append(QueryString.Create(queryData));
                ...
                var data = await response.Content.ReadFromJsonAsync<ResponseData<ListModel<Device>>>
